...
import io.goldstone.blinnnk.common.utils.UIUtils.setGradientColor
	...
	fun setStyle(
		...
		shaderStyle = when (type) {
			...
			GradientType.DarkGreen -> LinearGradient(
				0f, 0f, ScreenSize.Width.toFloat(), distance.toFloat(),
				intArrayOf(
					Color.parseColor("#FF204E4D"),
					Color.parseColor("#FF40857B")
				),
				floatArrayOf(0f, distance * 1f),
				Shader.TileMode.CLAMP
			)
			GradientType.DarkGreenYellow -> LinearGradient(
				0f, 0f, ScreenSize.Width.toFloat(),
				distance.toFloat(),
				intArrayOf(
					Color.parseColor("#FF045855"),
					Color.parseColor("#FF2E320B")
				),
