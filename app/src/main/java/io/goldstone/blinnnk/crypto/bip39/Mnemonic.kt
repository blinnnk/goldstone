package io.goldstone.blinnnk.crypto.bip39

import io.goldstone.blinnnk.crypto.bip32.ExtendedKey
import io.goldstone.blinnnk.crypto.bip32.generateKey
import io.goldstone.blinnnk.crypto.extensions.sha256
import io.goldstone.blinnnk.crypto.extensions.toBitArray
import io.goldstone.blinnnk.crypto.extensions.toByteArray
...
object Mnemonic {
	...
	private fun mnemonicToEntropy(words: Array<String>): ByteArray {
		...
		for ((phraseIndex, word) in words.withIndex()) {
			val dictIndex = Collections.binarySearch(MnemonicWordList, word)
			if (dictIndex < 0)
	...
	fun entropyToMnemonic(entropy: ByteArray): String {
		...
		for (i in 0 until numWords) {
			...
			words.add(MnemonicWordList[index])
		}
