...
public class TransactionSignature extends ECKey.ECDSASignature {
   ...
   */
  public static org.bitcoinj.crypto.TransactionSignature dummy() {
    BigInteger val = ECKey.HALF_CURVE_ORDER;
    return new org.bitcoinj.crypto.TransactionSignature(val, val);
  }
  ...
  public static int calcSigHashValue(Transaction.SigHash mode, boolean anyoneCanPay, boolean useForkId) {
    Preconditions.checkArgument(SigHash.ALL == mode || SigHash.NONE == mode || SigHash.SINGLE == mode || SigHash.FORKID == mode); // enforce compatibility since this code was made before the SigHash enum was updated
    int sighashFlags = mode.value;
    if (anyoneCanPay)
      sighashFlags |= Transaction.SigHash.ANYONECANPAY.value;
    if(useForkId)
  ...
  public ECKey.ECDSASignature toCanonicalised() {
    return new org.bitcoinj.crypto.TransactionSignature(super.toCanonicalised(), sigHashMode(), anyoneCanPay(), useForkId());
  }
   ...
   */
  @Deprecated
  public static org.bitcoinj.crypto.TransactionSignature decodeFromBitcoin(byte[] bytes,
  ...
  public static org.bitcoinj.crypto.TransactionSignature decodeFromBitcoin(byte[] bytes,
    ...
    return new org.bitcoinj.crypto.TransactionSignature(sig.r, sig.s, bytes[bytes.length - 1]);
  }
}
