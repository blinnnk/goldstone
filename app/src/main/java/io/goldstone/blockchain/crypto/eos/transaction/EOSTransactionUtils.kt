...
object EOSTransactionUtils {
	fun serialize(
		...
		val serializedExpirationDate = EOSUtils.getExpirationCode(ExpirationType.generate(expirationDate))
		val serializedHeader = TransactionHeader.serialize(header)
		val serializedDelaySecond = EOSUtils.getVariableUInt(delaySecond)
		...
		authorizations.forEach { authorization ->
			serializedAuthorizations += EOSUtils.getLittleEndianCode(authorization.actor) + EOSUtils.getLittleEndianCode(authorization.permission)
		}
		...
		val serializedData = chainID.id + serializedExpirationDate + serializedHeader + serializedDelaySecond +
	...
	private fun String.completeZero(): String {
		val willCompleteSize = 314 - length
		var completeZero = ""
		(0 until willCompleteSize).forEach {
			completeZero += 0
		}
		return this + completeZero
	}
}