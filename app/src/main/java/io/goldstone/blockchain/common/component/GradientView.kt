...
class GradientView(context: Context) : View(context) {
	...
	fun setStyle(
		type: GradientType,
		distance: Int = context.getRealScreenHeight().orZero()
	) {
		shaderStyle = when (type) {
			GradientType.Blue -> LinearGradient(
				0f, 0f, ScreenSize.Width.toFloat(), distance.toFloat(), intArrayOf(
					Color.parseColor("#FF122750"), Color.parseColor("#FF204972"),
					Color.parseColor("#FF0e6c8c"), Color.parseColor("#FF0e6c8c")
				), floatArrayOf(0f, 0.3f, 0.8f, 1f), Shader.TileMode.MIRROR
			)
			GradientType.PinkToYellow -> setGradientColor(
				Color.parseColor("#FF00FF80"), Color.parseColor("#FF0076FF")
			)
			GradientType.BlueGray -> setGradientColor(
				Color.parseColor("#FF1E3950"), Color.parseColor("#FF24353E")
			)
			GradientType.BlueGreen -> LinearGradient(
				0f, 0f, ScreenSize.Width.toFloat(), distance.toFloat(),
			...
			GradientType.BlueGreenHorizontal -> LinearGradient(
				0f, 0f, distance.toFloat(), height.toFloat(),
				intArrayOf(Color.rgb(0, 120, 242), Color.rgb(2, 209, 142)), floatArrayOf(0f, distance * 1f),
				Shader.TileMode.CLAMP
			)
			GradientType.DarkGreen -> LinearGradient(
				0f, 0f, ScreenSize.Width.toFloat(), distance.toFloat(),
				intArrayOf(Color.parseColor("#FF204E4D"), Color.parseColor("#FF40857B")),
				floatArrayOf(0f, distance * 1f), Shader.TileMode.CLAMP
			)
